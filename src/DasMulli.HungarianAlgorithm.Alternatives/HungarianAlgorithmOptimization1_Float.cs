...
{
    /// <summary>
    /// Class HungarianAlgorithmOptimization1_Float.
    ///
    ...
    public static class HungarianAlgorithmOptimization1_Float
    {
        /// <summary>
        ...
        public static int[] FindAssignments([NotNull] Matrix<double> assignmentCostsDoubles)
        {
            ...
            var costs = assignmentCostsDoubles.ToSingle();
            ...
            for (var i = 0; i < rows; i++)
            {
                for (var j = 0; j < columns; j++)
                {
                    if (Math.Abs(costs[i, j]) <= 0 && !rowsCovered[i] && !colsCovered[j])
                    {
                        masks[i, j] = 1;
        ...
        private static bool TryFindZero([NotNull] Matrix<float> costs, [NotNull] bool[] rowsCovered, [NotNull] bool[] colsCovered, out Location zeroLocation)
        {
            if (costs == null)
            {
